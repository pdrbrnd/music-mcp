...
export const libraryGetCurrentTrackTool: Tool = {
  ...
  description:
    "📚 YOUR LIBRARY: Get information about the currently playing track",
  inputSchema: {
    type: "object",
    properties: {},
    required: [],
...
export const librarySearchTool: Tool = {
  name: "library_search",
  description:
    "📚 YOUR LIBRARY: Search your music library for tracks, albums, or artists",
  inputSchema: {
    type: "object",
    properties: {
      query: {
...
export const libraryBrowseTool: Tool = {
  name: "library_browse",
  description:
    "📚 YOUR LIBRARY: Browse your music library by type (tracks, albums, artists, playlists)",
  inputSchema: {
    type: "object",
    properties: {
      type: {
  ...
  try {
    ...
    switch (input.type) {
      case "tracks":
        ...
        result = await executeAppleScript(
          join(__dirname, "../scripts/library/search-tracks.applescript"),
          ["", limit.toString()], // Empty search returns all, with limit
          config.timeoutSeconds * 1000,
        );
        break;
